...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['Tutorial_clear', 'Hell_unlock', 'CrossedOut_Normal_clear', 'CrossedOut_Hard_clear',
				'CrossedOut_Hell_clear', 'CrossedOut_Normal_nomiss', 'CrossedOut_Hard_nomiss', 'CrossedOut_Hell_nomiss',
				'tutorial_secret', 'tutorial_supersecret']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'Tutorial_clear' | 'CrossedOut_Normal_clear' | 'CrossedOut_Hard_clear' | 'Hell_unlock' | 'CrossedOut_Hell_clear':
						if(!changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{		
								case 'CrossedOut_tutorial':
									if(achievementName == 'Tutorial_clear') unlock = true;
								case 'CrossedOut_Normal':
									if(achievementName == 'CrossedOut_Normal_clear') unlock = true;
								case 'CrossedOut_Hard':
									if(achievementName == 'CrossedOut_Hard_clear') unlock = true;
									if(achievementName == 'Hell_unlock') unlock = true;
								case 'CrossedOut_Hell':
									if(achievementName == 'CrossedOut_Hell_clear') unlock = true;
							}
						}
					case 'CrossedOut_Normal_nomiss' | 'CrossedOut_Hard_nomiss' | 'CrossedOut_Hell_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'CrossedOut_Normal':
									if(achievementName == 'CrossedOut_Normal_nomiss') unlock = true;
								case 'CrossedOut_Hard':
									if(achievementName == 'CrossedOut_Hard_nomiss') unlock = true;
								case 'CrossedOut_Hell':
									if(achievementName == 'CrossedOut_Hell_nomiss') unlock = true;
							}
						}
